...
struct osrfx2 {
	...
	spinlock_t		dev_lock;		/* lock for the device */
	/* all device attributes which will be be accessed in both process 
	 * context and interrupt context should be guarded by dev_lock
	*/
	...
	__u8			bulk_in_addr;		/* the address of the bulk in endpoint */
	size_t			bulk_in_maxp;		/* max packet size of the bulk in endpoint */
	 ...
	 */
	__u8			bulk_out_addr;		/* the address of the bulk out endpoint */
	/**
	...
	/* Track all inflight urbs for write in case we need to retract our submissions.
	 * More info about anchor, refer to:
	 * (1): anchors and their use @http://www.spinics.net/lists/linux-usb-devel/msg05522.html
	 * (2): [patch 0/5] introduction of usb_anchor and overhaul of usb-skeleton
	 * 	@http://www.spinics.net/lists/linux-usb-devel/msg05943.html
	 */
...
static int osrfx2_find_endpoints(struct osrfx2 *fx2dev)
{
	...
	for (i = 0; i < iface_desc->desc.bNumEndpoints; i++) {
		...
		if (!fx2dev->bulk_in_addr &&
		    usb_endpoint_dir_in(endpoint) &&
		    usb_endpoint_xfer_bulk(endpoint)) {
			/* we found a bulk in endpoint */
			fx2dev->bulk_in_addr = endpoint->bEndpointAddress;
			/* since 3.2, we need to use usb_endpoint_maxp() */
			fx2dev->bulk_in_maxp = endpoint->wMaxPacketSize; 
		}

		if (!fx2dev->bulk_out_addr &&
		      ...
		      struct usb_interface *interface)
{
	...
	/* Bulk IN */
	/* need not kzallc, just allocate memory */
	fx2dev->bulk_in_buffer = kmalloc(fx2dev->bulk_in_maxp, GFP_KERNEL);
...
static int osrfx2_fp_read_submit_urb(struct osrfx2 *fx2dev, size_t count)
{
	...
	usb_fill_bulk_urb(fx2dev->bulk_in_urb,
			  fx2dev->udev,
			  usb_rcvbulkpipe(fx2dev->udev, fx2dev->bulk_in_addr),
			  fx2dev->bulk_in_buffer,
			  min(fx2dev->bulk_in_maxp, count),
			...
			loff_t          *ppos)
{
	...
	usb_fill_bulk_urb(urb,
			  fx2dev->udev,
			  usb_sndbulkpipe(fx2dev->udev, fx2dev->bulk_out_addr),
			  buf,
			  writesize,
